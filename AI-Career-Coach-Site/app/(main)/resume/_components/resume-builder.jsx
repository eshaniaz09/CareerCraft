...
export default function ResumeBuilder({ initialContent }) {
  const [cvData, setCVData] = useState(initialContent ? JSON.parse(initialContent) : defaultCVData);
  const [activeTab, setActiveTab] = useState('edit');
  const isMobile = useIsMobile();

  // Load saved CV from localStorage
  useEffect(() => {
    // If we have initial content from the server, we don't need to load from localStorage
    ...
    try {
      ...
      if (savedData) {
        // Only update if different to avoid infinite loops or render issues
  ...
  // Save CV to localStorage and database whenever it changes
  useEffect(() => {
    localStorage.setItem('cv-data', JSON.stringify(cvData));

    const saveToDb = async () => {
      try {
        await saveResume(JSON.stringify(cvData));
      } catch (error) {
        ...
        <div className="min-h-screen bg-gradient-to-br from-violet-50 to-indigo-50">
          ...
          <Tabs defaultValue="edit" value={activeTab} onValueChange={setActiveTab} className="w-full h-full">
            <div className="p-4 border-b bg-white/90 backdrop-blur-md sticky top-0 z-10 shadow-sm">
              <h1 className="text-2xl font-bold text-center mb-4 text-transparent bg-clip-text bg-gradient-to-r from-violet-600 to-indigo-600">
                ImpactCV
              </h1>
          ...
          <div className="bg-white/40 backdrop-blur-md rounded-xl border shadow-lg overflow-hidden transition-all hover:shadow-xl">
            <ResizablePanelGroup direction="horizontal" className="min-h-[800px]">
              ...
              <ResizablePanel defaultSize={70} className="transition-all">
                <div className="h-full bg-gray-300/75 backdrop-blur-sm flex items-center justify-center p-8 overflow-auto">
                  <Preview data={cvData} />
                </div>
              </ResizablePanel>
            </ResizablePanelGroup>
